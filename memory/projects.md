...
- [OpenClaw 运维] Daily Reflection 已按 `/home/re0hg/.openclaw/workspace` 作为统一 Git 根连续验证 commit/push 可用；但当前 `git add memory` 仍会带入 `memory/_state/`、`heartbeat-state.json`、`memory/weekly/` 与 archive 重命名等自动产物，下一步应改成“仅人工维护记忆文件”的显式白名单。
